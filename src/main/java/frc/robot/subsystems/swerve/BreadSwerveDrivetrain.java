 ...
 */
public class BreadSwerveDrivetrain {
  protected final boolean IsOnCANFD;
  protected final CANBus canbus = new CANBus(TunerConstants.DrivetrainConstants.CANbusName);
  ...
  protected final StatusSignal<Angle> m_yawGetter;
  protected final StatusSignal<AngularVelocity> m_angularVelocity;
  ...
  protected BreadSwerveRequest m_requestToApply = new BreadSwerveRequest.Idle();
  protected SwerveControlRequestParameters m_requestParameters =
  ...
  protected boolean checkIsOnCanFD() {
    return canbus.isNetworkFD();
  }
   ...
   */
  public BreadSwerveDrivetrain(
      LegacySwerveDrivetrainConstants driveTrainConstants, LegacySwerveModuleConstants... modules) {
   ...
   *
   * @param driveTrainConstants Drivetrain-wide constants for the swerve drive
   * @param OdometryUpdateFrequency The frequency to run the odometry loop. If unspecified, this is
  ...
  public BreadSwerveDrivetrain(
    ...
    int iteration = 0;
    for (LegacySwerveModuleConstants module : modules) {
      Modules[iteration] = new BreadSwerveModule(module, driveTrainConstants.CANbusName);
  ...
  public void tareEverything() {
    try {
      ...
      m_ShotOdometry.resetPosition(
          Rotation2d.fromDegrees(m_yawGetter.getValueAsDouble()), m_modulePositions, new Pose2d());
  ...
  /**
   * Takes the {@link SwerveRequest.ForwardReference#RedAlliance} perpective direction and treats it
   * as the forward direction for {@link SwerveRequest.ForwardReference#OperatorPerspective}.
   *
   ...
   *     SwerveRequest.ForwardReference#RedAlliance} perspective.
   */
   ...
   */
  public BreadSwerveModule getModule(int index) {
   ...
   *
   * <p>This can also be used for logging data if the function performs logging instead of telemetry
   *
