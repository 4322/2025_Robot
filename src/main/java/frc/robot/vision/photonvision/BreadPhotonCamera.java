...
public class BreadPhotonCamera implements AutoCloseable {
  ...
  public double lastCall = 0.0;
  public double lastCallTwo = 0.0;
  public double lastEstimate = 0.0;
  public double lastCaptureTime = 0;
   ...
   */
  public BreadPhotonCamera(NetworkTableInstance instance, String cameraName) {
    name = cameraName;
   ...
   */
  public BreadPhotonCamera(String cameraName) {
  ...
  public PhotonPipelineResult getLatestResult() {
    ...
    double currentCall = Timer.getFPGATimestamp();
  ...
  //      double currentCall = Timer.getFPGATimestamp();
  //      var currentQueue = resultSubscriber.getAllValues();
  //      ArrayList<PhotonPipelineResult> unprocessedResults = new ArrayList<>();

  //      for (int i = 0; i < currentQueue.size(); i++) {
  //          if (lastEstimate == 0.0) {
  //              lastEstimate = currentCall;
  //          } else {
  //             double captureTime = currentQueue.get(i).value.getCaptureTimestampMicros() / 1e6;
  //             lastEstimate = lastEstimate + (captureTime - lastCaptureTime);
  //             lastCaptureTime = captureTime;
  ...
  //         currentQueue.get(i).value.setRecieveTimestampMicros((long) (lastEstimate * 1e6));

  //         unprocessedResults.add(currentQueue.get(i).value);
  //      }
  ...
  //         double resultTimestamp = result.timestamp / 1000000.0; // Convert from microseconds to
  // seconds
