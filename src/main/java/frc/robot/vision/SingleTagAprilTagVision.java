...
public class SingleTagAprilTagVision extends SubsystemBase {
    ...
    public void setDataInterfaces(Supplier<Pose2d> poseSupplier, Consumer<List<TimestampedVisionUpdate>> visionConsumer) {
    ...
    public void periodic() {
        ...
        int targetTagID = RobotContainer.operatorBoard.getAprilTag();
        ...
        List<PhotonPipelineResult> frontLeftCamUnprocessedResults = frontLeftCamera.getAllUnreadResults();
        List<PhotonPipelineResult> frontRightCamUnprocessedResults = frontRightCamera.getAllUnreadResults();

        // Values of camera being used
        List<PhotonPipelineResult> unprocessedResults = useFrontLeftCam ? frontLeftCamUnprocessedResults : frontRightCamUnprocessedResults;
        Pose3d robotToCameraPose = useFrontLeftCam ? Constants.Vision.frontLeftCamera3dPos : Constants.Vision.frontRightCamera3dPos;
        ...
        for (PhotonPipelineResult unprocessedResult : unprocessedResults) {
            ...
            double xyStdDev = xyStdDevModel.predict(tagPos.getTranslation().getDistance(cameraPose.getTranslation()));
